...
export function MelodyWheels() {
  ...
  /**
   * The backdrop video must never make a sound — the only audio on this page is
   * the song, and two soundtracks at once is not a mix anyone chose.
   *
   * The JSX prop is not the whole story. React has historically treated `muted`
   * as a DOM property and left it out of server-rendered HTML entirely, which
   * leaves a window before hydration where the element is not muted at all.
   * (This React does emit the attribute — the built HTML was checked — but it is
   * one upgrade away from not, and the cost of being sure is three lines.)
   * `defaultMuted` writes the `muted` ATTRIBUTE, which is what autoplay policies
   * read and what survives a restore from the back/forward cache.
   *
   * Playback is then started by hand, because a muted autoplay promise can still
   * reject, and an unhandled rejection logs an error for something purely
   * decorative.
   */
  useEffect(() => {
    ...
    // A looping moving image is precisely what this setting asks us not to
    // serve, so hold the first frame instead — it is the illustration anyway.
    ...
    void video.play().catch(() => {
      // Blocked despite being muted (a data-saver mode, usually). The first
      // frame still paints, so the page falls back to a still backdrop.
    });
  }, []);
      ...
      */}
      <div className={isPlaying ? "is-playing" : undefined}>
        <div className="pointer-events-none fixed inset-0 z-0" aria-hidden>
          {/* The same illustration, now moving: the bus rolls and its wheels
              turn. Silent by construction — see the effect above for why the
              `muted` prop alone is not something to trust. */}
          <video
            ref={backdropRef}
            src="/melody-wheels.mp4"
            muted
            autoPlay
            loop
            playsInline
            ...
            tabIndex={-1}
            className="backdrop absolute inset-0 h-full w-full object-cover object-center"
          />
          ...
          {/* Scrim, in three passes rather than one flat wash:
              ...
              white type sits in the same light as the illustration. */}
